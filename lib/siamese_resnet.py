...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, pretrained=True, cut_at_pooling=False,
                ...
                last_conv_stride=1, skip_classification=True, fc2_relu=True, fc_init='default'):
        ...
        self.fc2_relu = fc2_relu

        # Construct base (pretrained) resnet
        if depth not in ResNet.__factory:
            raise KeyError("Unsupported depth:", depth)
        if type(self.depth) is str and  's1' in self.depth:
            self.base = ResNet.__factory[depth](pretrained=pretrained, last_conv_stride=last_conv_stride)
        else:
