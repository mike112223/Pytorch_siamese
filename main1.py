...
def test(model):
    ...
    for j in range(blen):
        ...
        im_a = crop_image(object1, int(l)).copy()
        im_b = crop_image(object2, int(l)).copy()

        batch_im_a = pre_test(im_a)
        batch_im_b = pre_test(im_b)
...
def pre_test(img, target_size, mean, std):
    ### resize
